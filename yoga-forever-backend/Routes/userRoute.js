...
router.post("/user", (req, res) => {
  ...
  const { name, age, phone, startDate, feesPaid, batchNumber } = req.body;
  ...
  if (
    ...
    //Perform validation check for age,phone & date
    ...
    //If phone and age both are invalid
    else if (isPhoneValid == false && (age < 18 || age > 65)) {
      res
        .status(401)
        .json({
          message: "Invalid phone address and invalid age",
          message_id: "InEmInAg",
        });
      return;
    }
    //If only phone is invalid
    else if (isPhoneValid == false) {
      res
        .status(401)
        .json({ message: "Invalid phone address", message_id: "InEm" });
      return;
    }
      ...
      .then((savedUser) => {
        ...
        if (savedUser) {
          ...
          if (Days_difference > 30) {
            userCollection
              .updateOne({ phone: phone }, { $set: { startDate: currentDate } })
              .then((updatedUser) => {
                res
                  .status(200)
                  .json({
